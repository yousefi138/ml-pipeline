...
##
Goal: Add feature names to reports including consolidated_results report

Currently, the reports generated by the pipeline provide excellent information about hyperparameter values and ranking of most important predictive features in the best performing models. However, somehow the names of those important predictive features haven't made it to the report.

Can you update so that feature names are inhereted and displayed as provided in input data during the results reporting?

## 
Goal: for the parameters used in imputation in the test folds to come entirely from estimates generated in the training set. 

For example, I suspect that the 'median' used by the median imputation is currently being estimated as the median value as estimated in the test set. Instead, I would like the pipeline to use the median estimated in the training set and apply that parameter in the test set.
